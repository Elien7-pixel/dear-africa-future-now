...
const AddNarrativeSovereigntyStatic = () => {
  ...
  const imageUrl = '/lovable-uploads/95da0cb0-54a4-407d-a107-fcae3d3d3fbf.png';

  const handleInsert = async () => {
    setIsSubmitting(true);
    try {
      // Insert article referencing public image path (no storage upload)
      ...
      toast.success('Article created successfully (static image path)');
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <main className="container mx-auto px-4 py-10">
      <h1 className="text-2xl font-bold mb-4">Add: Narrative Sovereignty (Static Image)</h1>
      <p className="mb-6">This admin-only helper creates the article using a public image path so it works after copying dist to Hostinger.</p>
      <div className="flex items-center gap-3 mb-8">
        <Button onClick={handleInsert} disabled={isSubmitting} className="bg-african-orange hover:bg-african-orange/90">
          {isSubmitting ? 'Working...' : 'Create Article (Static Image)'}
        </Button>
        <a href={imageUrl} target="_blank" rel="noreferrer" className="text-african-blue underline">
          View public image path
        </a>
      </div>
...
export default AddNarrativeSovereigntyStatic;
